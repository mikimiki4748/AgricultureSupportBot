...
def getEnvData():
    ...
    node_id = '7'#7 15
    keys = '[\"nodeID\",\"time\",\"air_temperature\",\"illuminance\",\"ATPR\",\"amount_of_solar_radiation\",\"wind_speed\",\"wind_direction\",\"rainfall\",\"precipitation\"]'
    ...
    try:
        ...
        f.write(',ATPR,_id,air_temperature,amount_of_solar_radiation,nodeID,precipitation,relative_humidity,time,wind_direction,wind_speed\n')
        id = 0
        dt = ''
        ...
        for datum in env_list:
            ...
            #print(datum)
            dec = dict(datum)
            f.write(str(dec.get('ATPT',''))+',')
            f.write(str(dec.get('_id',''))+',')
            f.write(str(dec.get('air_temperature',''))+',')
            f.write(str(dec.get('amount_of_solar_radiation',''))+',')
            f.write(str(dec.get('nodeID',''))+',')
            f.write(str(dec.get('precipitation',''))+',')
            f.write(str(dec.get('relative_humidity',''))+',')
            #9時間プラス
            str_dt = dec.get('time','')
            if str_dt != '':
            ...
            f.write(str(dec.get('wind_direction',''))+',')
            f.write(str(dec.get('wind_speed','')))
            f.write('\n')
            id += 1
        f.close()
